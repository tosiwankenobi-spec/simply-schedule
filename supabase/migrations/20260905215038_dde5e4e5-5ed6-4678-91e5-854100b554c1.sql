-- Forward corrective migration: restores the hardened planner routines that a
-- later migration had replaced with weaker versions, and tightens them further.
-- Both keep SECURITY INVOKER + empty search_path so row level security and the
-- caller's own identity always apply.
...
BEGIN
  ...
  -- The same block may never appear twice in one plan.
  IF (
  ...
  -- Approving the same proposal twice returns the first result.
  ...
  -- Lock the affected part of this person's schedule in a fixed order, so two
  -- approvals running at once can never interleave or deadlock.
  FOR v_lock_id IN
  ...
  LOOP
    ...
    END IF;
    -- The new time must belong to the day being planned (allowing for the
    -- caller's time zone offset either side of midnight).
    ...
    END IF;
    -- A block that is now a fixed commitment, or is not a task block, never moves.
    ...
    IF NOT FOUND OR v_task.id <> (v_move->>'taskId')::uuid THEN
      RAISE EXCEPTION 'Your schedule changed. Check your day again before approving.';
    END IF;
    -- A move must not push work past its own deadline.
  ...
  -- With every move written, the resulting day must be free of overlaps: moved
  -- blocks against each other and against everything left in place.
  ...
  EXCEPTION WHEN unique_violation THEN
    -- Another approval of the same proposal won the race; keep its result and
    -- discard this one entirely.
...
BEGIN
  ...
  -- Lock every affected block in a fixed order first.
  FOR v_lock_id IN
  ...
  LOOP
    ...
    ELSIF v_row.starts_at = (v_change->>'toStart')::timestamptz
      ...
      AND v_row.is_all_day = false THEN
      UPDATE public.appointments
    ...
    ELSE
      -- Newer edits, and anything now protected, are left exactly as they are.
