-- Restore the Replan Preview + Undo database contract on projects where the
-- original Feature 2 migrations were committed but never applied. This remains
-- safe on fresh databases where those migrations already ran.
...
REVOKE ALL ON public.plan_runs FROM PUBLIC;
REVOKE ALL ON public.plan_runs FROM anon;
REVOKE ALL ON public.plan_runs FROM authenticated;
...
DROP POLICY IF EXISTS "Users read their own plan runs" ON public.plan_runs;
DROP POLICY IF EXISTS "Users create their own plan runs" ON public.plan_runs;
DROP POLICY IF EXISTS "Users update their own plan runs" ON public.plan_runs;
DROP POLICY IF EXISTS "Users delete their own plan runs" ON public.plan_runs;
...
CREATE INDEX IF NOT EXISTS plan_runs_user_applied_idx
  ON public.plan_runs (user_id, applied_at DESC);
...
DROP TRIGGER IF EXISTS plan_runs_set_updated_at ON public.plan_runs;
...
BEGIN
  ...
  LOOP
    ...
    -- Times and versions are serialized at full microsecond precision so undo
    -- can compare them losslessly against the stored row.
    v_changes := v_changes || jsonb_build_object(
      'appointmentId', v_row.id,
      'taskId', v_task.id,
      'title', v_row.title,
      'source', v_row.source,
      'fromStart', to_char(v_row.starts_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),
      'fromEnd', to_char(v_row.ends_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),
      'toStart', to_char(v_to_start AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),
      'toEnd', to_char(v_to_end AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),
      'appliedVersion', to_char(v_applied_version AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_row.starts_at = (v_change->>'toStart')::timestamptz
      ...
      -- changes the row's version and is preserved instead of undone. The
      -- comparison is lossless: versions are stored with microsecond precision.
      AND (
...
GRANT EXECUTE ON FUNCTION public.apply_day_replan(uuid, date, jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.undo_plan_run(uuid) TO service_role;
