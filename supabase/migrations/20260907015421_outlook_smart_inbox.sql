-- Outlook Smart Inbox is independently pausable from Outlook Calendar.
ALTER TABLE public.sync_settings
  ADD COLUMN IF NOT EXISTS outlook_mail_sync_enabled boolean NOT NULL DEFAULT true;

-- Keep the unified timeline label provider-specific while retaining the
-- invoker security required for underlying appointments/household RLS.
