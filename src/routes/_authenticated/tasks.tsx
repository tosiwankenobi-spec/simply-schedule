...
import { CheckCircle2, Circle, Trash2, Wand2, Plus, Clock } from "lucide-react";
  ...
  head: () => ({
    meta: [
      { title: "Task backlog · Chronos-V" },
      {
        name: "description",
        content:
          "Capture tasks with priority, effort and deadlines, then let AI auto-schedule them into your free time.",
      },
      { property: "og:title", content: "Task backlog · Chronos-V" },
      {
        property: "og:description",
        content: "Capture tasks and let AI auto-schedule them into your free time.",
      },
...
function TasksPage() {
  ...
  const status = useMutation({
    mutationFn: (v: { id: string; status: "open" | "scheduled" | "done" }) =>
      setTaskStatus({ data: v }),
  ...
  async function runPreview() {
    setBusy(true);
    try {
      const res = await autoScheduleTasks({
        data: { date, dryRun: true, taskIds, tzOffsetMin: new Date().getTimezoneOffset() },
      });
      setPreview(res);
      if (res.placements.length === 0)
        toast.message("No room in that day's free time for those tasks.");
    } catch (e) {
  ...
  async function apply() {
    ...
    try {
      const res = await autoScheduleTasks({
        data: { date, dryRun: false, taskIds, tzOffsetMin: new Date().getTimezoneOffset() },
      });
      setPreview(null);
      setSelected([]);
      invalidate();
      toast.success(
        `Scheduled ${res.placements.length} task${res.placements.length === 1 ? "" : "s"}`,
      );
    } catch (e) {
  ...
  return (
    <div className="verolane-wash relative min-h-screen bg-background">
      ...
      <div className="relative mx-auto max-w-[1180px] px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
        <WorkspaceHeader
          eyebrow="Task + calendar fusion"
          title={
            <>
              Turn intentions into <span className="text-accent italic">time.</span>
            </>
          }
          description="Capture what needs doing, then let Chronos-V place focused work into the gaps your profile allows."
        />
        ...
        <div className="mt-8 grid items-start gap-8 xl:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <aside className="rounded-2xl border border-border bg-card/90 p-5 shadow-[0_18px_45px_rgba(0,46,40,0.05)] xl:sticky xl:top-8">
            <div className="flex items-center gap-2 text-accent">
              <Wand2 className="h-4 w-4" />
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em]">AI placement</p>
            </div>
            <h2 className="mt-2 font-serif text-xl">Smart scheduling</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Pick the tasks you want placed — or leave all unticked to consider the whole backlog.
              Blocks land in the free gaps inside your working hours, longest deadline pressure and
              ...
              <div className="mt-4">
                <div className="flex items-baseline justify-between">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setSelected(selected.length === open.length ? [] : open.map((t) => t.id));
                      setPreview(null);
                    }}
                  >
                    ...
                    {preview.placements.map((p) => (
                      <li key={p.task_id} className="flex items-center justify-between text-sm">
                        ...
                        <span className="shrink-0 text-muted-foreground">
                          {format(new Date(p.starts_at), "h:mm a")} –{" "}
                          {format(new Date(p.ends_at), "h:mm a")}
                        </span>
                      </li>
                    ))}
                  </ul>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap gap-x-3 text-xs text-muted-foreground">
                  ...
                  {t.deadline && (
                    <span>due {format(new Date(`${t.deadline}T00:00:00`), "MMM d")}</span>
                  )}
                </div>
              </div>
              <button
...
function NewTaskForm({ onSaved }: { onSaved: () => void }) {
  ...
  return (
    <form
      onSubmit={submit}
      className="space-y-3 rounded-2xl border border-border bg-card/90 p-5 shadow-[0_18px_45px_rgba(0,46,40,0.04)]"
    >
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="Draft the Q3 proposal"
          maxLength={200}
          required
        />
      </div>
