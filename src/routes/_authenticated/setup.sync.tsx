    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-3xl px-6 py-10">
        <Button asChild variant="ghost" size="sm" className="mb-6 -ml-2 text-muted-foreground">
          <Link to="/app">
            <ArrowLeft className="h-4 w-4 mr-1.5" /> Back to schedule
          </Link>
        </Button>

        <h1 className="text-3xl font-semibold tracking-tight">Calendar sync</h1>
        <p className="mt-2 text-muted-foreground">
          Control which Google Calendars flow into Chronos-V, who wins when both sides change, and watch
          exactly what each sync run did.
        </p>
        ...
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>
              Applies when the same event was changed in both Google Calendar and Chronos-V since the last sync.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            >
              {POLICIES.map((p) => (
                <div key={p.value} className="flex items-start gap-3 rounded-lg border p-3">
        ...
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              Only the calendars you tick are pulled in. New appointments you create here are written to your
              primary calendar.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {calendars.isLoading ? (
              ...
              <div className="space-y-2">
                {(calendars.data ?? []).map((c) => {
                  ...
                  const checked = selected.includes(c.id) || (c.primary && selected.includes("primary"));
                  return (
                    <label
                      key={c.id}
                      className="flex cursor-pointer items-center gap-3 rounded-lg border p-3 hover:bg-secondary/40"
                    >
                      <Checkbox
                        checked={checked}
          ...
          <CardContent className="space-y-4">
              ...
              {(status.data?.calendars ?? []).map((c) => (
                <div key={c.calendarId} className="rounded-lg border p-3 text-sm">
                  ...
                  <p className="mt-1 text-muted-foreground">
                    ...
                    {c.pagesSynced === 1 ? "" : "s"}
                    {c.lastSyncedAt ? ` · ${format(new Date(c.lastSyncedAt), "MMM d, h:mm a")}` : ""}
                  </p>
            ...
            <div className="space-y-1.5">
              {(status.data?.log ?? []).map((entry) => (
                <div key={entry.id} className="flex items-start gap-2 rounded-md border px-3 py-2 text-sm">
                  {entry.level === "error" ? (
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={entry.level === "error" ? "text-destructive" : ""}>{entry.message}</p>
