...
import { ArrowLeft, CalendarArrowUp, FileCheck2, HardDrive, Laptop, Trash2 } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        <div className="flex items-center gap-2 text-accent">
          ...
          <span className="text-sm font-medium">Calendar import</span>
        </div>
        <h1 className="mt-2 font-serif text-4xl text-foreground">Bring every calendar together.</h1>
        <p className="mt-3 max-w-2xl text-muted-foreground">
          Export an iCalendar file from Outlook, Apple Calendar, Android, or another calendar app.
          Chronos-V previews it on this device before anything is saved.
        </p>
        ...
        <Card className="mt-8">
          ...
          <CardContent className="space-y-5">
                ...
                <div className="space-y-2">
                  {preview.events.slice(0, 6).map((event) => (
                    <div
                      ...
                      className="flex items-start justify-between gap-3 rounded-md bg-background px-3 py-2 text-sm"
                    >
            ...
            <div className="flex items-start gap-2 rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground">
              ...
              <p>
                The raw file stays on this device. After approval, Chronos-V stores only normalized
                event details in your private account. Reimporting the same events updates them.
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {imports.isLoading ? (
              ...
              <p className="text-sm text-destructive">
                {imports.error instanceof Error ? imports.error.message : "Couldn't load imports."}
              </p>
            ) : imports.data?.length ? (
